...
def highlight_code_with_shiki(code: str, lang: str) -> str:
    """
    Highlight code using Shiki via Node.js subprocess.
    Falls back to plain escaped code if Shiki fails.
    """
    highlight_script = SCRIPTS_DIR / "highlight.mjs"
    
    if not highlight_script.exists():
        print(f"  Warning: Shiki highlight script not found, using plain code")
        return plain_code_block(code, lang)
    
    try:
        result = subprocess.run(
            ["node", str(highlight_script), lang, SHIKI_THEME],
            input=code,
            capture_output=True,
            text=True,
            timeout=30
        ...
        if result.returncode == 0:
            ...
            print(f"  Warning: Shiki error for {lang}: {result.stderr}")
            return plain_code_block(code, lang)
            
    except subprocess.TimeoutExpired:
        print(f"  Warning: Shiki timeout for {lang}")
        return plain_code_block(code, lang)
    except Exception as e:
        print(f"  Warning: Shiki failed: {e}")
        return plain_code_block(code, lang)


def plain_code_block(code: str, lang: str) -> str:
    """Fallback plain code block without highlighting."""
    escaped = (code
        .replace("&", "&amp;")
        .replace("<", "&lt;")
        .replace(">", "&gt;"))
    return f'<pre class="shiki" style="background-color:#24292e;color:#e1e4e8" tabindex="0"><code>{escaped}</code></pre>'


def process_code_blocks(md_content: str) -> str:
    """
    Extract and highlight code blocks before markdown processing.
    Replaces fenced code blocks with highlighted HTML wrapped in placeholders.
    """
    placeholders = {}
    counter = [0]
    ...
    def replace_code_block(match):
        ...
        # Highlight the code
        highlighted = highlight_code_with_shiki(code, lang)
        
        # Wrap in our code-block-wrapper with copy button
...
{highlighted}
        ...
        # Store in placeholders to protect from markdown processing
        placeholder = f"<!--CODE_BLOCK_{counter[0]}-->"
        placeholders[placeholder] = wrapped
        counter[0] += 1
...
def restore_code_blocks(html_content: str, placeholders: dict) -> str:
...
def fix_image_paths(html_content: str) -> str:
    ...
    return re.sub(r'src="\.\.\/images\/', 'src="../../images/', html_content)
...
def generate_toc_html(toc: list[dict]) -> str:
    ...
    for item in toc:
        ...
        html_parts.append(
            ...
            f'<a href="#{item["id"]}">{html_module.escape(item["text"])}</a>'
            f'</li>'
        )
...
def convert_markdown_to_html(md_content: str) -> tuple[str, dict]:
    """
    Convert markdown to HTML.
    Code blocks are pre-processed with Shiki, then restored after markdown conversion.
    """
    # First, extract and highlight code blocks
    processed_content, placeholders = process_code_blocks(md_content)
    
    # Convert markdown (without code highlighting - we handle that separately)
    ...
    # Restore the highlighted code blocks
    html_content = restore_code_blocks(html_content, placeholders)
...
def render_post(template: str, metadata: dict, content_html: str, toc_html: str, reading_time: int) -> str:
    ...
    if metadata.get("tags"):
        tags_list = ' &bull; '.join(metadata["tags"])
        tags_html = f'<span class="post-tags">{tags_list}</span>'
    
    rendered = template.replace("{{title}}", html_module.escape(metadata.get("title", "Blog Post")))
    rendered = rendered.replace("{{date}}", html_module.escape(metadata.get("date", "")))
    ...
    rendered = rendered.replace("{{description}}", html_module.escape(metadata.get("description", "")))
...
def update_blog_listing():
    """Update blog.html to link to static HTML files."""
...
def check_node_available():
    """Check if Node.js is available for Shiki."""
    try:
    ...
    print("Warning: Node.js not found. Syntax highlighting will use fallback.")
    return False


def check_shiki_installed():
    """Check if Shiki npm package is installed."""
    node_modules = Path("node_modules/shiki")
    if node_modules.exists():
        return True
    
    print("Installing Shiki...")
    try:
        result = subprocess.run(
            ["npm", "install", "shiki"],
            capture_output=True,
        ...
        print(f"Failed to install Shiki: {e}")
        return False
...
def main():
    ...
    node_ok = check_node_available()
    if node_ok:
        shiki_ok = check_shiki_installed()
        if not shiki_ok:
            print("Warning: Shiki not available, using plain code blocks")
